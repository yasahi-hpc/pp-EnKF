...
public:
  LBM2D()=delete;
  LBM2D(sycl::queue& queue, Config& conf, IOConfig& io_conf) : Model(queue, conf, io_conf) {}
  ~LBM2D() override {}
...
public:
  ...
  void initialize(std::unique_ptr<DataVars>& data_vars) {
    ...
    // Allocate data
    vor_     = RealView2D(queue_, "vor", nx, ny);
    nu_      = RealView2D(queue_, "nu", nx, ny);
    vor_obs_ = RealView2D(queue_, "vor_obs", nx, ny);
    noise_   = RealView2D(queue_, "noise", nx, ny);

    RealView2D p(queue_, "p", nkx, nkx);
    RealView2D theta(queue_, "theta", nkx, nkx);
    ...
    Impl::for_each(queue_, policy2d, init_fluid_moments);

    // Modify u, v by max. vs cfl
    value_type u_ref = static_cast<value_type>(conf_.phys_.u_ref_);
    
    usm_alloc<value_type> alloc(queue_);
    usm_vector<value_type> max_speed(1, 0, alloc);
    max_speed.at(0) = 1e-30 * u_ref;
    //value_type* max_speed_ptr;
    //max_speed_ptr = sycl::malloc_shared<value_type>(1, queue_);
    //*max_speed_ptr = 1e-30 * u_ref;
    ...
    queue_.submit([&](sycl::handler& cgh) {
        ...
        auto max_speed_reduction = sycl::reduction(max_speed.data(), sycl::maximum<value_type>());
      #else
        auto max_speed_reduction = sycl::reduction(max_speed.data(), sycl::maximum<value_type>(), sycl::property::reduction::initialize_to_identity{});
      #endif
      cgh.parallel_for<class reduction_kernel>(
        nd_range, max_speed_reduction,
        [=](sycl::nd_item<2> item, auto& max_speed_reduction) {
          const int ix = item.get_global_id(0);
          const int iy = item.get_global_id(1);

          auto _u = u(ix, iy);
          auto _v = v(ix, iy);
          max_speed_reduction.combine(std::sqrt(_u*_u + _v*_v));
        }
      );
    }).wait();

    value_type vmax = max_speed.at(0);
    //value_type vmax = *max_speed_ptr;
    //sycl::free(max_speed_ptr, queue_);
    ...
    */
    ...
    Impl::for_each(queue_, policy3d, init_feq_functor(conf_, rho, u, v, f));

    // Initialize force term
    force_ = std::move( std::unique_ptr<Force>(new Force(queue_, conf_)) );
...
private:
  ...
  void streaming(std::unique_ptr<DataVars>& data_vars) {
    ...
    Iterate_policy<3> policy3d({0, 0, 0}, {nx, ny, Q});
    Impl::for_each(queue_, policy3d, streaming_functor(conf_, f, fn));
  }
  ...
  void macroscopic(std::unique_ptr<DataVars>& data_vars) {
    ...
    Iterate_policy<2> policy2d({0, 0}, {nx, ny});
    Impl::for_each(queue_, policy2d, macroscopic_functor(conf_, fn, rho, u, v));
  }
  ...
  void streaming_macroscopic(std::unique_ptr<DataVars>& data_vars) {
    ...
    Iterate_policy<2> policy2d({0, 0}, {nx, ny});
    Impl::for_each(queue_, policy2d, streaming_macroscopic_functor(conf_, f, fn, rho, u, v));
  }
  ...
  void sgs(std::unique_ptr<DataVars>& data_vars) {
    ...
    Iterate_policy<2> policy2d({0, 0}, {nx, ny});
    Impl::for_each(queue_, policy2d, sgs_functor(conf_, rho, u, v, nu));
  }
  ...
  void collision(std::unique_ptr<DataVars>& data_vars) {
    ...
    Iterate_policy<2> policy2d({0, 0}, {nx, ny});
    Impl::for_each(queue_, policy2d, collision_srt_functor(conf_, fn, fx, fy, rho, u, v, nu));
  }
...
private:
  void inspect(std::unique_ptr<DataVars>& data_vars, const int it) {
    ...
    usm_alloc<value_type> alloc(queue_);
    usm_vector<value_type> moments(9, 0, alloc);

    #if defined(__HIPSYCL__) || defined(__OPENSYCL__)
      auto reduce_momentum_x = sycl::reduction(moments.data()    , sycl::plus<value_type>());
      auto reduce_momentum_y = sycl::reduction(moments.data() + 1, sycl::plus<value_type>());
      auto reduce_energy     = sycl::reduction(moments.data() + 2, sycl::plus<value_type>());
      auto reduce_enstrophy  = sycl::reduction(moments.data() + 3, sycl::plus<value_type>());
      auto reduce_nus        = sycl::reduction(moments.data() + 4, sycl::plus<value_type>());
      auto reduce_mass       = sycl::reduction(moments.data() + 5, sycl::plus<value_type>());
      auto reduce_divu2      = sycl::reduction(moments.data() + 6, sycl::plus<value_type>());
      auto reduce_divu       = sycl::reduction(moments.data() + 7, sycl::plus<value_type>());
      auto reduce_vel2       = sycl::reduction(moments.data() + 8, sycl::plus<value_type>());
    #else
      auto reduce_momentum_x = sycl::reduction(moments.data()    , sycl::plus<value_type>(), sycl::property::reduction::initialize_to_identity{});
      auto reduce_momentum_y = sycl::reduction(moments.data() + 1, sycl::plus<value_type>(), sycl::property::reduction::initialize_to_identity{});
      auto reduce_energy     = sycl::reduction(moments.data() + 2, sycl::plus<value_type>(), sycl::property::reduction::initialize_to_identity{});
      auto reduce_enstrophy  = sycl::reduction(moments.data() + 3, sycl::plus<value_type>(), sycl::property::reduction::initialize_to_identity{});
      auto reduce_nus        = sycl::reduction(moments.data() + 4, sycl::plus<value_type>(), sycl::property::reduction::initialize_to_identity{});
      auto reduce_mass       = sycl::reduction(moments.data() + 5, sycl::plus<value_type>(), sycl::property::reduction::initialize_to_identity{});
      auto reduce_divu2      = sycl::reduction(moments.data() + 6, sycl::plus<value_type>(), sycl::property::reduction::initialize_to_identity{});
      auto reduce_divu       = sycl::reduction(moments.data() + 7, sycl::plus<value_type>(), sycl::property::reduction::initialize_to_identity{});
      auto reduce_vel2       = sycl::reduction(moments.data() + 8, sycl::plus<value_type>(), sycl::property::reduction::initialize_to_identity{});      
    #endif
    ...
    queue_.submit([&](sycl::handler& cgh) {
      cgh.parallel_for<class moments_kernel>(
        nd_range, reduce_momentum_x, reduce_momentum_y, 
        reduce_energy, reduce_enstrophy, reduce_nus,
        reduce_mass, reduce_divu2, reduce_divu, reduce_vel2,
        [=](sycl::nd_item<2> item,
            auto& reduce_momentum_x,
            auto& reduce_momentum_y,
            auto& reduce_energy,
            auto& reduce_enstrophy,
            auto& reduce_nus,
            auto& reduce_mass,
            auto& reduce_divu2,
            auto& reduce_divu,
          ...
          reduce_momentum_x += (tmp_rho * tmp_u);
          reduce_momentum_y += (tmp_rho * tmp_v);
          reduce_energy     += ( tmp_u * tmp_u + tmp_v * tmp_v );
          reduce_nus        += nu(ix, iy);
          reduce_mass       += tmp_rho;
          ...
          reduce_enstrophy += ( (vx - uy) * (vx - uy) + (ux + vy) * (ux + vy) ) / 2;
          reduce_divu      += (ux + vy);
          reduce_divu2     += ( (ux + vy) * (ux + vy) );
          reduce_vel2      += ( tmp_u * tmp_u + tmp_v * tmp_v );
        }
      );
    }).wait();

    usm_vector<value_type> min_max(4, 0, alloc);
    min_max.at(3) = 9999; // Some large number
    #if defined(__HIPSYCL__) || defined(__OPENSYCL__)
      auto reduce_maxvel2 = sycl::reduction(min_max.data()    , sycl::maximum<value_type>());
      auto reduce_maxdivu = sycl::reduction(min_max.data() + 1, sycl::maximum<value_type>());
      auto reduce_rho_max = sycl::reduction(min_max.data() + 2, sycl::maximum<value_type>());
      auto reduce_rho_min = sycl::reduction(min_max.data() + 3, sycl::minimum<value_type>());
    #else
      auto reduce_maxvel2 = sycl::reduction(min_max.data()    , sycl::maximum<value_type>(), sycl::property::reduction::initialize_to_identity{});
      auto reduce_maxdivu = sycl::reduction(min_max.data() + 1, sycl::maximum<value_type>(), sycl::property::reduction::initialize_to_identity{});
      auto reduce_rho_max = sycl::reduction(min_max.data() + 2, sycl::maximum<value_type>(), sycl::property::reduction::initialize_to_identity{}); 
      auto reduce_rho_min = sycl::reduction(min_max.data() + 3, sycl::minimum<value_type>(), sycl::property::reduction::initialize_to_identity{});   
    #endif

    queue_.submit([&](sycl::handler& cgh) {
      cgh.parallel_for<class maximums_kernel>(
        nd_range, reduce_maxvel2, reduce_maxdivu, reduce_rho_max, reduce_rho_min,
        [=](sycl::nd_item<2> item,
            auto& reduce_maxvel2,
            auto& reduce_maxdivu,
            auto& reduce_rho_max,
          ...
          reduce_maxdivu.combine( std::abs(ux + vy) );
          reduce_maxvel2.combine(tmp_u * tmp_u + tmp_v * tmp_v);
          reduce_rho_max.combine(tmp_rho);
          reduce_rho_min.combine(tmp_rho);
        }
      );
    }).wait();
    ...
    #if defined(__HIPSYCL__) || defined(__OPENSYCL__)
      auto _sycl_reduction = std::bind(sycl::reduction, std::placeholders::_1, sycl::maximum<value_type>());
    #else
      auto _sycl_reduction = std::bind(sycl::reduction, std::placeholders::_1, sycl::maximum<value_type>(), sycl::property::reduction::initialize_to_identity{});
    #endif

      auto momentum_x = _sycl_reduction(moments.data() );
      auto momentum_y = _sycl_reduction(moments.data() + 1);
      auto energy     = _sycl_reduction(moments.data() + 2);
      auto enstrophy  = _sycl_reduction(moments.data() + 3);
      auto nus        = _sycl_reduction(moments.data() + 4);
      , enstrophy, nus, mass, divu2, divu, vel2
    ...
    */
    ...
    auto momentum_x_total = moments.at(0) / (nx * ny);
    auto momentum_y_total = moments.at(1) / (nx * ny);
    auto energy           = moments.at(2) / (nx * ny);
    auto enstrophy        = moments.at(3) / (nx * ny);
    auto nus_total        = moments.at(4) / (nx * ny);
    auto mass             = moments.at(5) / (nx * ny);
    auto divu2            = moments.at(6) / (nx * ny);
    auto divu             = moments.at(7) / (nx * ny);
    auto vel2             = moments.at(8) / (nx * ny);

    auto maxdivu = min_max.at(0);
    auto maxvel2 = min_max.at(1);
    auto rho_max = min_max.at(2);
    auto rho_min = min_max.at(3);
  ...
  void update_vorticity(const ViewType& u, const ViewType& v, ViewType& vor) {
    ...
    Iterate_policy<2> policy2d({0, 0}, {nx, ny});
    Impl::for_each(queue_, policy2d, vorticity_functor(conf_, u_tmp, v_tmp, vor_tmp));
  }
