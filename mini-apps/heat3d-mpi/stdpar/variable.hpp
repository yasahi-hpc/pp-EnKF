...
private:
  ...
  std::vector<RealType> x_mask_, y_mask_, z_mask_;
...
public:
  ...
  inline RealView1D x_mask() {
    return RealView1D( x_mask_.data(), extents_x_ );
  }

  inline RealView1D y_mask() {
    return RealView1D( y_mask_.data(), extents_y_ );
  }

  inline RealView1D z_mask() {
    return RealView1D( z_mask_.data(), extents_z_ );
  }
};
