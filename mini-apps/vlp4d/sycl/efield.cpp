...
// Field init
Efield::Efield(sycl::queue& q, Config& conf) {
  auto nx = conf.dom_.nxmax_[0], ny = conf.dom_.nxmax_[1];
  rho_ = RealView2D(q, "rho", nx, ny);
  ex_  = RealView2D(q, "ex",  nx, ny);
  ey_  = RealView2D(q, "ey",  nx, ny);
  phi_ = RealView2D(q, "phi", nx, ny);
  ...
  fft_ = std::make_unique<fft_type>(nx, ny, 1);
  fft_->set_stream(q);
  int nx1h = fft_->nx1h();

  rho_hat_ = ComplexView2D(q, "rho_hat", nx1h, ny);
  ex_hat_  = ComplexView2D(q, "ex_hat", nx1h, ny);
  ey_hat_  = ComplexView2D(q, "ey_hat", nx1h, ny);

  filter_  = RealView1D(q, "filter", nx1h);
...
void Efield::solve_poisson_fftw(sycl::queue& q) {
  float64 kx0 = 2 * M_PI / xmax_;
  ...
  Impl::for_each(q, policy1d, solve_poisson);
