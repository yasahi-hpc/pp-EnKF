...
static inline void lag_basis(const double px, double * coef) {
  ...
  struct advect_1D_x_functor {
    ...
    void operator()(const std::size_t idx) const {
      const std::size_t ix   = idx % s_nxmax;
      const std::size_t iyvx = idx / s_nxmax;
      const std::size_t iy   = iyvx % s_nymax;
      const std::size_t ivx  = iyvx / s_nymax;
  ...
  void advect_1D_x(const Config& conf, Scheduler&& scheduler, const RealView4D& fn, RealView4D& fnp1, float64 dt) {
    const Domain dom = conf.dom_;
    auto [s_nxmax, s_nymax, s_nvxmax, s_nvymax] = dom.nxmax_;
    const auto _fn = fn.mdspan();
    auto _fnp1 = fnp1.mdspan();

    auto advect_1D_x_task = stdexec::just()
      | exec::on( scheduler, stdexec::bulk(s_nxmax*s_nymax*s_nvxmax, advect_1D_x_functor(conf, _fn, _fnp1, dt)) );
    stdexec::sync_wait( std::move(advect_1D_x_task) );
  };

  template <class Scheduler>
  void advect_1D_y(const Config& conf, Scheduler&& scheduler, const RealView4D& fn, RealView4D& fnp1, float64 dt) {
    const Domain dom = conf.dom_;
    auto [s_nxmax, s_nymax, s_nvxmax, s_nvymax] = dom.nxmax_;
    const auto _fn = fn.mdspan();
    auto _fnp1 = fnp1.mdspan();

    auto advect_1D_y_task = stdexec::just()
      | exec::on( scheduler, stdexec::bulk(s_nxmax*s_nymax*s_nvxmax, advect_1D_y_functor(conf, _fn, _fnp1, dt)) );
    stdexec::sync_wait( std::move(advect_1D_y_task) );
  };

  template <class Scheduler>
  void advect_1D_vx(const Config& conf, Scheduler&& scheduler, const RealView4D& fn, RealView4D& fnp1, Efield* ef, float64 dt) {
    const Domain dom = conf.dom_;
    auto [s_nxmax, s_nymax, s_nvxmax, s_nvymax] = dom.nxmax_;
    auto ex = ef->ex();
    const auto _fn = fn.mdspan();
    ...
    auto advect_1D_vx_task = stdexec::just()
      | exec::on( scheduler, stdexec::bulk(s_nxmax*s_nymax*s_nvxmax, advect_1D_vx_functor(conf, _fn, _fnp1, ex, dt)) );
    stdexec::sync_wait( std::move(advect_1D_vx_task) );
  };
  
  template <class Scheduler>
  void advect_1D_vy(const Config& conf, Scheduler&& scheduler, const RealView4D& fn, RealView4D& fnp1, Efield* ef, float64 dt) {
    const Domain dom = conf.dom_;
    auto [s_nxmax, s_nymax, s_nvxmax, s_nvymax] = dom.nxmax_;
    auto ey = ef->ey();
    const auto _fn = fn.mdspan();
    ...
    auto advect_1D_vy_task = stdexec::just()
      | exec::on( scheduler, stdexec::bulk(s_nxmax*s_nymax*s_nvxmax, advect_1D_vy_functor(conf, _fn, _fnp1, ey, dt)) );
    stdexec::sync_wait( std::move(advect_1D_vy_task) );
  };
