#ifndef LETKF_SOLVER_HPP
#define LETKF_SOLVER_HPP
...
public:
  LETKFSolver(sycl::queue& queue, const letkf_config_type& letkf_config) : queue_(queue) {
...
public:
  void solve() {
    ...
    // Ensemble average
    Impl::mean(queue_, X, x_mean, 1); // (n_stt, n_ens, n_batch) -> (n_stt, 1, n_batch)
    Impl::mean(queue_, Y, y_mean, 1); // (n_obs, n_ens, n_batch) -> (n_obs, 1, n_batch)

    // dX = X - mean(X), dY = Y - mean(Y)
    Impl::axpy(queue_, X, x_mean, dX, -1); // (n_stt, n_ens, n_batch) - (n_stt, 1, n_batch) -> (n_stt, n_ens, n_batch)
    Impl::axpy(queue_, Y, y_mean, dY, -1); // (n_obs, n_ens, n_batch) - (n_obs, 1, n_batch) -> (n_obs, n_ens, n_batch)

    // dyo = yo - mean(Y)
    Impl::axpy(queue_, yo, y_mean, -1); // (n_obs, 1, n_batch)
    ...
    Impl::deep_copy(queue_, I, Q); // (n_ens, n_ens, n_batch)
    ...
    auto P = P_.mdspan();
    Impl::diag(queue_, d, inv_D, -1); // (n_ens, n_ens, n_batch)
    ...
    Impl::diag(queue_, d, inv_D, -0.5); // (n_ens, n_ens, n_batch)
    Impl::matrix_matrix_product(blas_handle_, inv_D, V, tmp_ee, "N", "T"); // (n_ens, n_ens, n_batch) * (n_ens, n_ens, n_batch) -> (n_ens, n_ens, n_batch)
    ...
    Impl::axpy(queue_, W, w); // (n_ens, n_ens, n_batch) + (n_ens, 1, n_batch) -> (n_ens, n_ens, n_batch)
    Impl::matrix_matrix_product(blas_handle_, dX, W, X, "N", "N"); // (n_stt, n_ens, n_batch) * (n_ens, n_ens, n_batch) -> (n_stt, n_ens, n_batch)
    //blas_handle_.wait();
    Impl::axpy(queue_, X, x_mean); // (n_stt, n_ens, n_batch) + (n_stt, 1, n_batch) -> (n_stt, n_ens, n_batch)
  }

private:
  void initialize(sycl::queue& queue) {
    // Allocate Views
    X_  = RealView3D(queue, "X", n_stt_, n_ens_, n_batch_);
    dX_ = RealView3D(queue, "dX", n_stt_, n_ens_, n_batch_);
    Y_  = RealView3D(queue, "Y", n_obs_, n_ens_, n_batch_);
    dY_ = RealView3D(queue, "dY", n_obs_, n_ens_, n_batch_);

    x_mean_ = RealView3D(queue, "x_mean", n_stt_, 1, n_batch_);
    y_mean_ = RealView3D(queue, "y_mean", n_obs_, 1, n_batch_);

    yo_ = RealView3D(queue, "yo", n_obs_, 1, n_batch_);

    I_     = RealView3D(queue, "I", n_ens_, n_ens_, n_batch_);
    Q_     = RealView3D(queue, "Q", n_ens_, n_ens_, n_batch_);
    V_     = RealView3D(queue, "V", n_ens_, n_ens_, n_batch_);
    d_     = RealView2D(queue, "d", n_ens_, n_batch_);
    inv_D_ = RealView3D(queue, "inv_D", n_ens_, n_ens_, n_batch_);
    P_     = RealView3D(queue, "P", n_ens_, n_ens_, n_batch_);

    rR_ = RealView3D(queue, "rR", n_obs_, n_obs_, n_batch_);
    w_  = RealView3D(queue, "w", n_ens_, 1, n_batch_);
    W_  = RealView3D(queue, "W", n_ens_, n_ens_, n_batch_);

    tmp_ee_ = RealView3D(queue, "tmp_ee", n_ens_, n_ens_, n_batch_);
    tmp_oe_ = RealView3D(queue, "tmp_oe", n_obs_, n_ens_, n_batch_);
    tmp_o_  = RealView2D(queue, "tmp_o", n_obs_, n_batch_);
    tmp_e_  = RealView2D(queue, "tmp_e", n_ens_, n_batch_);

    auto rR = rR_.mdspan();
    auto I = I_.mdspan();
    Impl::identity(queue, rR);
    Impl::identity(queue, I);

    auto d = d_.mdspan();
    auto V = V_.mdspan();
    blas_handle_.create();
    syevj_handle_.create(V, d);
    blas_handle_.set_stream(queue);
    syevj_handle_.set_stream(queue);
  }
};
