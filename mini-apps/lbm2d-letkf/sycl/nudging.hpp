...
public:
  Nudging(sycl::queue& queue, Config& conf, IOConfig& io_conf) : DA_Model(queue, conf, io_conf) {}
  Nudging(sycl::queue& queue, Config& conf, IOConfig& io_conf, MPIConfig& mpi_conf)=delete;
  virtual ~Nudging(){}
  void initialize() {
    setFileInfo();
  }
  ...
  void apply(std::unique_ptr<DataVars>& data_vars, const int it, std::vector<Timer*>& timers){
    ...
    Impl::for_each(queue_, policy2d, nudging_functor(conf_, rho_obs, u_obs, v_obs, f));
