...
public:
  void initialize(sycl::queue& q, int* argc, char*** argv){
    // Load args
    ...
    // Allocate attributes
    data_vars_ = std::move( std::unique_ptr<DataVars>(new DataVars(q, conf_)) );
    model_     = std::move( model_factory(sim_type_, q, conf_, io_conf_) );
    da_model_  = std::move( da_model_factory(sim_type_, q, conf_, io_conf_, mpi_conf_) );
